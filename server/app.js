...
app.get('/progress-tracker', (req, res) => {
    const dashboard = fs.readFileSync(path.join(__dirname, '..', 'public', 'progress-tracker.html'), 'utf-8');
    res.send(template.replace('{{content}}', dashboard));
});


app.get('/submission', (req, res) => {
    const dashboard = fs.readFileSync(path.join(__dirname, '..', 'public', 'document-submission.html'), 'utf-8');
    res.send(template.replace('{{content}}', dashboard));
});
